...
/// Декодированный payload из Telegram OIDC id_token.
/// ВНИМАНИЕ: парсинг НЕ проверяет подпись JWT. Использовать только на клиенте-моке.
/// На бэкенде нужно верифицировать JWT по JWKS `https://oauth.telegram.org/.well-known/jwks.json`.
