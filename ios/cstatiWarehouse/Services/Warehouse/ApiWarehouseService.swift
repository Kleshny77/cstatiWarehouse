...
final class ApiWarehouseService: WarehouseServiceProtocol {
    ...
    private static func mapError(_ error: APIError) -> WarehouseError {
        ...
        case .unauthorized:
            return .unauthorized
        case .server(let status, let code, let message, let body):
            if status == 409, code == "item_version_conflict", let body,
               let decoded = try? JSONDecoder.apiWarehouseDecoder.decode(ItemVersionConflictBodyDTO.self, from: body),
               let serverItem = decoded.item.toItem() {
                return .concurrentModification(serverItem)
            }
            switch code {
            case "not_found":
                return .notFound
...
private extension JSONDecoder {
    static var apiWarehouseDecoder: JSONDecoder {
...
private struct UpdateItemRequestDTO: Encodable {
    ...
    init(item: Item) {
        ...
        self.measureUnit = item.measureUnit.rawValue
        self.volumePerUnit = item.volumePerUnit
        self.expectedUpdatedAt = item.updatedAt
    }
}

private struct ItemVersionConflictBodyDTO: Decodable {
    let error: String?
    let message: String?
    let item: ItemDTO
}
