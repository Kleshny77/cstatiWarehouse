...
struct ItemEditView: View {
    ...
    private var form: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            if !presenter.hidesQuantityStepper {
                ...
                Text("Остаток и единица учёта задаются у каждого варианта отдельно.")
            ...
            if presenter.showsMeasureFields {
                measureUnitPicker
                GlassTextField(
                    title: presenter.amountPerPackageFieldTitle,
                    placeholder: presenter.amountPerPackagePlaceholder,
            ...
            .onChange(of: presenter.draft.locationAddress) { _, _ in
                presenter.locationAddressChanged()
            }
            AddressGeocodeInlinePreview(
                status: presenter.addressGeocodePreviewStatus,
                manualCheckHint: "Адрес проверяется автоматически после паузы во вводе — ниже появится карта с меткой.",
                secondaryLabelColor: .white.opacity(0.82),
                tertiaryLabelColor: .white.opacity(0.55)
            )
            holderField
            expirationField
        }
    ...
    private var categoryChipsFlow: some View {
...
#Preview("Редактирование позиции") {
    Text("Откройте через приложение (координатор)")
