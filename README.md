...
**Структура репозитория:** в корне рядом лежат **`backend/`** (сервер Go) и **`ios/`** (клиент: открывать **`ios/cstatiWarehouse.xcodeproj`**). Дополнительно: **`Docs/`**, **`scripts/`**.
...
- **Позиции (Item)** — товары/материалы: название, описание, категория, количество (**упаковок**), срок годности, фото, статус (`inStock` / `archived` с причиной), опционально «держатель» (`heldByUserID`), адрес хранения. Дополнительно: **единица меры** (`piece` / `liter` / `milliliter` / `kilogram` / `gram`), **`volume_per_unit`** — сколько единиц этой меры в **одной упаковке** (если не задано, на бэкенде трактуется как 1); для строк после миграции с **`milliliter` без `volume_per_unit`** клиент может показывать остаток как **суммарные мл** (плоский остаток), а не «упаковки по 1 мл». **Группа с подпозициями** — корневая карточка с массивом `variants`; в списке API подпозиции вложены в родителя, для литровых вариантов в стаке считается **`aggregated_volume_liters`**.
...
В репозитории есть папка **`backend/`** (Go) — контракт REST логично сверять с реализацией сервисов в **`ios/cstatiWarehouse/`** (`Api*Service` и протоколы).
...
Реализации лежат в `ios/cstatiWarehouse/Services/**/Api*Service.swift`, протоколы — `*Protocol.swift`, моки — `Mock*`.
...
- В таблице `items`: `parent_item_id` (FK на родителя, `ON DELETE CASCADE`), `variant_label`, `measure_unit` (`piece` | `liter` | `milliliter` | `kilogram` | `gram`), `volume_per_unit` — размер одной упаковки в выбранной мере (опционально; при явной передаче должно быть > 0). Устаревшие `package` / `meter` в старых заметках заменены на `piece` (см. миграции `00013`, `00016`, `00017`).
- **GET `/items`:** в JSON только **корневые** строки; у родителя с детьми — массив **`variants`** и при наличии литровых подпозиций в стаке — **`aggregated_volume_liters`** (сумма по вариантам с учётом `quantity` × эффективного `volume_per_unit` для л/мл).
...
Сторонние пакеты подключаются через проект (например **Lottie**, **TelegramLogin**) — точные версии смотреть в `ios/cstatiWarehouse.xcodeproj` / Package Dependencies.
